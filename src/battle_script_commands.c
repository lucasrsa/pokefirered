...
static void Cmd_accuracycheck(void)
{
    ...
    else
    {
        ...
        if (gBattleMons[gBattlerAttacker].ability == ABILITY_HUSTLE && IS_MOVE_PHYSICAL(gBattleMoves[move].category))
    ...
    if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
    {
        ...
        else
        {
            ...
            else // hp goes down
            {
                ...
                if (IS_MOVE_PHYSICAL(gBattleMoves[gCurrentMove].category) && !(gHitMarker & HITMARKER_PASSIVE_DAMAGE) && gCurrentMove != MOVE_PAIN_SPLIT)
                {
                ...
                else if (!IS_MOVE_PHYSICAL(gBattleMoves[gCurrentMove].category) && !(gHitMarker & HITMARKER_PASSIVE_DAMAGE))
                {
